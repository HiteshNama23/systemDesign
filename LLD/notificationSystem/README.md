...
**Flow**
    User → NotificationService → NotifierFactory → Notifier (Email/SMS/Push)
    ↘ ExecutorService → parallel send()
...
- `Used thread pool instead of creating a new thread per request (saves resources)`.
- `Could replace Factory with Dependency Injection (Spring) in real systems`.
- `Could replace in-memory executor with Kafka consumers for durability`.

## 13) Improvements & Interview Follow-ups
- `Add retry mechanism`.
- `Add scheduling (send later)`.
- `Multi-region deployment`.
- `Ask: “How do we guarantee exactly-once delivery?” → DB transaction + idempotency token`.